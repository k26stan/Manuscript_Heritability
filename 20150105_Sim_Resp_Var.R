...
# PathToSave <- "/Users/kstandis/Dropbox/Schork/JNJ11/Writing/Resp_Herit/Data/Variance/"
PathToSave <- "/Users/kstandis/Dropbox/Schork/JNJ11/Slides/20150105_Q3_Deliver/"
...
SIM.DAT <- function( N_IND, DIFF.GRP, N_TIME, TO_PLOT ) {
	...
	DEL.DAS.MN.A <- -2 # mean( FT$DEL_MNe_MN+DIFF.GRP, na.rm=T )
	DEL.DAS.MN.B <- -2+DIFF.GRP # mean( FT$DEL_MNe_MN, na.rm=T )
	...
	SIM.VAR.DEL.DAS.A <- sample( VAR.DAS.DIS, N_IND.A, replace=T )
	SIM.VAR.DEL.DAS.B <- sample( VAR.DAS.DIS, N_IND.B, replace=T )
	...
	for ( i in 1:N_IND.A ) {
		SIM.TIME.DAS.A[i,] <- rnorm( N_TIME, SIM.DEL.DAS.A[i], SIM.VAR.DEL.DAS.A[i] )
	}
	for ( i in 1:N_IND.B ) {
		SIM.TIME.DAS.B[i,] <- rnorm( N_TIME, SIM.DEL.DAS.B[i], SIM.VAR.DEL.DAS.B[i] )
	}
	X <- 1:10
	SIM.TIME.DAS.A[X,X]
	SIM.TIME.DAS.B[X,X]
	...
	if ( TO_PLOT==1 ) {
		...
		XLIM <- c( 1,N_TIME )
		YLIM <- c(-4,1) # range( SIM.TIME.DAS.A, SIM.TIME.DAS.B )
...
png( paste(PathToSave,"2_HER_SIM-4_Repeated_AB.png",sep=""), height=1000,width=1000,pointsize=28 )
TEST <- SIM.DAT(200, .1, 20, 1)
dev.off()
png( paste(PathToSave,"2_HER_SIM-4_Repeated_AK.png",sep=""), height=1000,width=1000,pointsize=28 )
TEST <- SIM.DAT(200, 1, 20, 1)
dev.off()
...
P.VALS <- numeric( N_TIME+1 )
P.VALS[1] <- t.test( DAT.1$Par$MN_A, DAT.1$Par$MN_B, paired=F, alternative="two.sided" )$p.value
for ( i in 1:N_TIME ) {
	P.VALS[i+1] <- t.test( DAT.1$Time$Time_A[,i], DAT.1$Time$Time_B[,i], paired=F, alternative="two.sided" )$p.value
}

## Plot P-Values for a pair of Groups
png( paste(PathToSave,"2_HER_SIM-5_Test_AB.png",sep=""), height=1000,width=1400,pointsize=34 )
...
axis( 1, at=1:length(P.VALS), label=c("Mean",2:length(P.VALS)-1), las=2 )
dev.off()
...
## Plot P-Values for a pair of Groups
png( paste(PathToSave,"2_HER_SIM-5_Test_AK.png",sep=""), height=1000,width=1400,pointsize=34 )
plot( 1:length(P.VALS), -log10(P.VALS), pch="+", col="chocolate2", main="Identifying Difference b/n Groups", xlab="Repeated Measures", ylab="-log10(p)", xaxt="n", type="b" )
abline( v=1:length(P.VALS), lty=2, col="grey50" )
...
for ( s in 1:length(SAMP_SIZE) ) {
	...
	for ( e in 1:length(EFF_SIZE) ) {
		eff <- EFF_SIZE[e]
		P.VALS[[s]][[e]] <- array(, c(N_ITER,N_TIME+1) )
		...
		for ( i in 1:N_ITER ) {
			...
			for ( t in 1:N_TIME ) {
				P.VALS[[s]][[e]][i,t+1] <- t.test( DAT$Time$Time_A[,t], DAT$Time$Time_B[,t], paired=F, alternative="two.sided" )$p.value
			} # Close Iter Loop
		} # Close Effect Size Loop
		colnames(P.VALS[[s]][[e]]) <- c( "MN", paste("t",1:N_TIME,sep="") )
	...
	print(paste( "Done with SAMPLE size:",size,"-",round( proc.time()-start, 2)[3] ))
}
names(P.VALS) <- paste("s",SAMP_SIZE,sep="")

## Go through Sample/Effect Sizes and Calculate Power
## Loop Through Sample Sizes
POWER.1 <- POWER.MN <- array( , c(length(SAMP_SIZE),length(EFF_SIZE)) )
rownames(POWER.1) <- rownames(POWER.MN) <- names(P.VALS)
colnames(POWER.1) <- colnames(POWER.MN) <- names(P.VALS[[1]])
start <- proc.time()
for ( s in 1:length(SAMP_SIZE) ) {
	size <- SAMP_SIZE[s]
	## Loop Thru Effect Sizes
	for ( e in 1:length(EFF_SIZE) ) {
		eff <- EFF_SIZE[e]
		POWER.MN[s,e] <- length(which( P.VALS[[s]][[e]][,1]<.05 )) / N_ITER
		POWER.1[s,e] <- length(which( P.VALS[[s]][[e]][,1:N_TIME+1]<.05 )) / N_ITER / N_TIME
	}
}

## Plot this Shiz
png( paste(PathToSave,"2_HER_SIM-6_Power_2.png",sep=""), height=1000,width=1400,pointsize=34 )
...
# abline( v=SAMPS_SIZE)
for ( e in 1 ) {
	points( SAMP_SIZE, POWER.MN[,e], col=COLS[e], lty=1, type="o", lwd=3, pch=20 )
	points( SAMP_SIZE, POWER.1[,e], col=COLS[e], lty=2, type="o", lwd=3, pch=20 )
}
legend( 20,1, legend=c("Mean","Single"), lty=c(1,2), lwd=3 )
dev.off()

png( paste(PathToSave,"2_HER_SIM-6_Power_ALL.png",sep=""), height=1000,width=1400,pointsize=34 )
COLS.list <- c("firebrick2","chocolate2","gold2","springgreen2","steelblue2","slateblue2")
# COLS.list <- c( "black", "firebrick1","chocolate1","gold1","springgreen1","cadetblue1","steelblue2","slateblue3")
COLS <- colorRampPalette(COLS.list)(length(EFF_SIZE))
XLIM <- range( SAMP_SIZE )
YLIM <- c(0,1)
...
legend( 80,.2, legend=c("Mean","Single"), lty=c(1,2), lwd=3 )
dev.off()



library(gplots)
heatmap.2( POWER.MN, Rowv=F, Colv=F, dendrogram="none",scale="none",trace="none" )
